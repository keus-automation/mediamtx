...
type keusSignalingServer struct {
	address string
	port    string
	token   string
}

type KeusSSRequest struct {
	// type string;
	sdp string
}
type KeusSSResponse struct {
	// Code int `json:"code"`
	success bool
}

func (s *keusSignalingServer) initialize() error {
	nc, err := nats.Connect("nats://"+s.address+":"+string(s.port), nats.Token(s.token))
	...
	_, subErr := nc.Subscribe("KEUS-ECHO-SHOW-PROVIDE-RPC", func(msg *nats.Msg) {
		fmt.Println("this is json rpc")
		fmt.Println(msg.Data)

		msg.Respond([]byte("hello, data"))
	})

	if subErr != nil {
		fmt.Println(subErr)
	}
	fmt.Println("Subscribed for echo show")
